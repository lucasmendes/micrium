...
const  NET_DEV_CFG_ETHER  NetDev_Cfg_A_1 = {
...
const  NET_PHY_CFG_ETHER  NetPhy_Cfg_A_1 = {
    1u,                             /* Phy bus address.                                                                 */
...
/*$PAGE*/
                                    /* ----------------- EXAMPLE ETHERNET DEVICE A, #2 CONFIGURATION ------------------ */

const  NET_DEV_CFG_ETHER  NetDev_Cfg_A_2 = {
    ...
    0x18000000u,                    /* Base address   of dedicated memory, if available.                                */
        0x8000u,                    /* Size           of dedicated memory, if available (in octets).                    */
   ...
   "00:AB:CD:EF:80:02",             /* Desired device hardware address; may be NULL address or string ...               */
                                    /* ... if  device hardware address configured or set at run-time.                   */
};



const  NET_PHY_CFG_ETHER  NetPhy_Cfg_A_2 = {
    2u,                             /* Phy bus address.                                                                 */
    NET_PHY_BUS_MODE_MII,           /* Phy bus mode.                                                                    */
    NET_PHY_TYPE_INT,               /* Phy type.                                                                        */
    NET_PHY_SPD_AUTO,               /* Auto-Negotiation determines link speed.                                          */
...
const  NET_DEV_CFG_PPP  NetDev_Cfg_B_1 = {
    ...
    NET_IF_MEM_TYPE_DEDICATED,      /* Desired receive  buffer memory pool type :                                       */
                                    /*   NET_IF_MEM_TYPE_MAIN        buffers allocated from main memory                 */
                                    /*   NET_IF_MEM_TYPE_DEDICATED   buffers allocated from (device's) dedicated memory */
    1500u,                          /* Desired size      of device's large receive  buffers (in octets) [see Note #2].  */
      10u,                          /* Desired number    of device's large receive  buffers             [see Note #3a]. */
       4u,                          /* Desired alignment of device's       receive  buffers (in octets) [see Note #4].  */
       0u,                          /* Desired offset from base receive  index, if needed   (in octets) [see Note #5a1].*/
